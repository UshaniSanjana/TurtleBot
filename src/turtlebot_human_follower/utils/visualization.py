...
class Visualizer:
    ...
    def draw_tracking_info(self, frame, position, distance, is_active, is_safe, fps=0.0):
        """
        ...
        cv2.circle(frame, (x, y), 6, color, -1)  # Reduced from 10
        cv2.circle(frame, (x, y), 10, color, 1)  # Reduced from 15, thickness 2→1
        
        # Draw distance text (smaller font)
        if distance:
            cv2.putText(frame, f"{distance:.2f}m", (x + 15, y),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)  # Size 0.7→0.5, thickness 2→1
        
        # Draw status (smaller font)
        cv2.putText(frame, status, (10, 25),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)  # Size 1.0→0.8
        
        # Draw FPS in top-right corner (smaller)
        if fps > 0:
            fps_text = f"FPS: {fps:.1f}"
            text_size = cv2.getTextSize(fps_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]  # Size 0.7→0.5, thickness 2→1
    ...
    def draw_gesture_hints(self, frame):
        ...
        for i, hint in enumerate(hints):
            cv2.putText(frame, hint, (10, y_offset + i * 18),  # Reduced spacing from 25
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)  # Size 0.6→0.4, thickness 2→1
